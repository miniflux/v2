...
import (
	...
	tgbotapi "github.com/kateile/telegram-bot-api/v6"
	"miniflux.app/model"
)

// PushEntry pushes entry to telegram chat using integration settings provided
func PushEntry(entry *model.Entry, botToken, chatID string, topicID string) error {
	...
	TopicIDInt, _ := strconv.ParseInt(topicID, 10, 64)
	msg := tgbotapi.NewMessage(chatIDInt, result.String())
	msg.ParseMode = tgbotapi.ModeHTML
	msg.DisableWebPagePreview = false
	msg.MessageThreadID = int(TopicIDInt)
