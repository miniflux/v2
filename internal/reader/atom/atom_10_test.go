...
func TestParseFeedWithRelativeURL(t *testing.T) {
	...
	want := "https://example.org/blog/atom.xml"
	if feed.FeedURL != want {
		t.Errorf("Incorrect feed URL, got: %s, want %s", feed.FeedURL, want)
	}
...
func TestParseEntryWithRelativeURL(t *testing.T) {
	...
	feed, err := Parse("http://example.net/", bytes.NewReader([]byte(data)))
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseInvalidXml(t *testing.T) {
	data := `garbage`
	_, err := Parse("http://example.net/", bytes.NewReader([]byte(data)))
	if err == nil {
