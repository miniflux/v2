...
func RefreshFeed(store *storage.Storage, userID, feedID int64, forceRefresh bool) *locale.LocalizedErrorWrapper {
	...
	if originalFeed.FeedSourceType == "web_scraper" {
		...
		// When JS rendering is enabled, use Lightpanda headless browser to render
		// the listing page first, then parse the rendered HTML with CSS selectors.
		// This handles pages where the item list is dynamically generated by JS.
		if originalFeed.UseJSRender && config.Opts.LightpandaEnabled() {
			slog.Debug("Rendering web scraper listing page with Lightpanda",
				slog.Int64("user_id", userID),
				slog.Int64("feed_id", feedID),
				slog.String("feed_url", originalFeed.FeedURL),
			)
			renderedHTML, renderErr := headless.RenderPageHTML(originalFeed.FeedURL, processor.ResolveProxyURLForHeadless(originalFeed), originalFeed.ID)
			if renderErr != nil {
				slog.Warn("Unable to render web scraper listing page with headless browser",
					slog.Int64("user_id", userID),
					slog.Int64("feed_id", feedID),
					slog.String("feed_url", originalFeed.FeedURL),
				...
				// issues (e.g. expired TLS certs) and mask the real rendering error.
				scrapeErr = fmt.Errorf("headless JS rendering failed: %w", renderErr)
