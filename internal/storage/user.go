...
func (s *Storage) CreateUser(userCreationRequest *model.UserCreationRequest) (*model.User, error) {
	...
	query := `
		...
		RETURNING
			...
			disable_bulk_operations_confirmations
	`
	...
	err = tx.QueryRow(
		...
		&user.DisableBulkOperationsConfirmations,
	)
	if err != nil {
		tx.Rollback()
	...
	if user.Password != "" {
		...
		query := `
			UPDATE users SET
				...
				disable_bulk_operations_confirmations=$31
			WHERE
				id=$32
		`
		...
		query := `
			UPDATE users SET
				...
				disable_bulk_operations_confirmations=$30
			WHERE
				id=$31
		`
...
func (s *Storage) UserByAPIKey(token string) (*model.User, error) {
	query := `
		SELECT
			...
			u.disable_bulk_operations_confirmations
		FROM
			users u
		LEFT JOIN
