...
func Serve(router *mux.Router, store *storage.Storage, pool *worker.Pool) {
	...
	sr.HandleFunc("/entries/{entryID}/summarize", handler.summarizeEntry).Methods(http.MethodPost)
	sr.HandleFunc("/ai/backfill", handler.backfillAISummaries).Methods(http.MethodPost)
	sr.HandleFunc("/ai/force-backfill", handler.forceBackfillAISummaries).Methods(http.MethodPost)
	sr.HandleFunc("/ai/page-summary", handler.generateAIPageSummary).Methods(http.MethodPost)
	sr.HandleFunc("/ai/backfill-status", handler.getBackfillStatus).Methods(http.MethodGet)
	sr.HandleFunc("/ai/stop-backfill", handler.stopBackfill).Methods(http.MethodPost)
